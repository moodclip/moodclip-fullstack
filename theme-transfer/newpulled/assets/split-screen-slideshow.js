(function () {
	const slideshow = () => {
		$(".split_screen_slideshow-section").each(function () {
			...
			const box = $(this).find(".split_screen_slideshow");
				...
				navigate(direction, targetIndex = null) {
					...
					this.items.forEach((item, index) => {
						if (index === this.current) {
							...
							item.classList.remove(
								"slideshow__text_prev",
								"slideshow__text_next"
							);
						} else if (
							index ===
							(this.current - direction + this.itemsTotal) % this.itemsTotal
						) {
							item.classList.add("slideshow__text_prev");
							item.classList.remove("slideshow__text_next");
						} else if (index === (this.current + direction) % this.itemsTotal) {
							item.classList.add("slideshow__text_next");
							item.classList.remove("slideshow__text_prev");
						} else {
							item.classList.remove(
								"slideshow__text_prev",
								"slideshow__text_next"
							...
							.timeline({
								...
								onComplete: () => {
									...
									if (this.syncCallback) {
										this.syncCallback(this.current);
									}
								},
							})
							.to(currentItem, {
			...
			const swiperText = () => {
				const swiperParms = {
					...
					on: {
						slideChange: () => {
							sliderBG.goTo(swiper.activeIndex);
						},
					},
				};
			...
			const navigate = (action) => {
				swiper.slideTo(sliderBG.current);
				sliderBG[action]();
			};
