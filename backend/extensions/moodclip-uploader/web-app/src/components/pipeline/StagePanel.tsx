...
export const StagePanel = ({
  ...
  const renderPrimaryButton = () => (
    <Button
      ...
      size="lg"
      className="w-full max-w-xs font-semibold text-xl px-10 py-6 rounded-3xl text-white border-0 transition-all duration-500 ease-out hover:scale-105 disabled:opacity-60"
  ...
  const renderBuilderButton = () => {
    if (stage.id !== 3 || !builderReady || !onLaunchBuilder) return null;
    return (
      <Button
  ...
  return (
    ...
    >
      ...
      <div className="text-center space-y-10">
        <div>
          <h2 className="font-heading text-6xl font-semibold mb-6 text-foreground">{stage.title}</h2>
          <p
            className={`text-2xl text-muted-foreground/80 transition-smooth ${stage.ctaStatus === 'running' ? 'animate-pulse' : ''}`}
          >
